...
type ConsentCategory = "necessary" | "externalServices";
type StoredConsent = Record<ConsentCategory, boolean>;

const DEFAULT_CONSENT: StoredConsent = {
  necessary: true,
...
function readCookieConsent(): StoredConsent | null {
  ...
  if (stored === "all") {
    return { necessary: true, externalServices: true };
  }
  ...
  try {
    const parsed = JSON.parse(stored) as Partial<StoredConsent> | null;
...
export function saveCookieConsent(nextConsent: Partial<StoredConsent>) {
...
export function CookieConsent() {
  ...
  const saveChoice = (externalServices: boolean) => {
    saveCookieConsent({ externalServices });
    setExternalServicesEnabled(externalServices);
    setIsOpen(false);
        ...
        <div className="grid gap-5 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            ...
            <p className="mt-2 text-sm leading-relaxed text-foreground/65">
              ...
              Externe Dienste wie Google Maps werden nur nach Ihrer Zustimmung
              geladen.
            </p>
            {showSettings && (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-foreground/60">
                        Für den Betrieb der Website erforderlich und immer
                        aktiv.
                      </p>
                    </div>
                ...
                <div className="rounded-2xl border border-border/50 bg-light/50 p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-foreground/60">
                        Erlaubt das Laden der Google-Maps-Karte.
                      </p>
                    </div>
                    <Switch
          ...
          <div className="flex flex-col gap-2 sm:flex-row md:flex-col">
            {showSettings ? (
              <Button
                className="rounded-full bg-[#00A8CC] text-white hover:bg-[#0096b8]"
                onClick={() => saveChoice(externalServicesEnabled)}
              >
                Auswahl speichern
              </Button>
            ) : (
